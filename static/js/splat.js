...
async function main() {
    ...
    // consruct URL from current URL path
    ...
    //     // "nike.splat",
    //     // location.href,
    //     params.get("url") || "2-12_seq0-6_2_1fps.splat",
    //     "https://huggingface.co/datasets/kleinfour/splat-data/resolve/main/",
    // );
    ...
    const frame = (now) => {
        ...
        if (carousel) {
            ...
            const t = Math.sin((Date.now() - start) / 4000);
            inv = translate4(inv, 1.5 * t, 0, 3 * (1 - Math.cos(t)));
            inv = rotate4(inv, -0.2 * t, 0, 2, 0);
